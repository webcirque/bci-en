...
echo "----------------------------------"
echo " "
echo "Better Cloudflare IPs (Translated)"
echo "Originally written by Badafans on GitHub"
echo "Translations done by EdChdX"
echo " "
echo "----------------------------------"
declare -i bandwidth
declare -i speed
echo " "
echo "Please enter your desired speed between you and Cloudflare network in Mbps."
echo "8 Mbps (8Mb/s) equals to 1MB/s."
read -p "Your desired speed: " bandwidth
...
do
	...
	do
		...
		do
			...
			then
				clear
				echo "[Phase 1/2] Batch ICMP latency testing."
				echo "Currently $p threads, $per % completed."
				sleep 0.5 
			else
			    clear
				echo "[Phase 1/2] Batch ICMP latency testing."
		...
		rm -rf icmp
		echo "[Phase 2/2] Batch speed testing."
		echo "Testing download speeds of the 100 most-reliable IPs from before."
		mkdir temp
		for i in `cat ip.txt`
		do
			echo $i Speed tesing started
		...
		echo "Waiting speedtest to finish. Will select 3 best servers."
		sleep 15
		echo Test compmeted.
		ls -S temp > ip.txt
		rm -rf temp
		...
		if [ $n -ge 3 ]; then
			...
			rm -rf ip.txt
			echo The best IPs are $first - $second - $third
			echo First attempt $first
			...
			max=$[$max/1024]
			echo Maximum speed is $max kB/s
			echo Second attempt $first
			curl --resolve speed.cloudflare.com:443:$first https://speed.cloudflare.com/__down?bytes=1000000000 -o /dev/null --connect-timeout 5 --max-time 15 > log.txt 2>&1
			cat log.txt | tr '\r' '\n' | awk '{print $NF}' | sed '1,3d;$d' | grep -v 'k\|M' >> speed.txt
			for i in `cat log.txt | tr '\r' '\n' | awk '{print $NF}' | sed '1,3d;$d' | grep k | sed 's/k//g'`
			do
			...
			max=$[$max/1024]
			echo Maximum speed $max kB/s
			echo Secomd attempt $second
			curl --resolve speed.cloudflare.com:443:$second https://speed.cloudflare.com/__down?bytes=1000000000 -o /dev/null --connect-timeout 5 --max-time 15 > log.txt 2>&1
			cat log.txt | tr '\r' '\n' | awk '{print $NF}' | sed '1,3d;$d' | grep -v 'k\|M' >> speed.txt
			for i in `cat log.txt | tr '\r' '\n' | awk '{print $NF}' | sed '1,3d;$d' | grep k | sed 's/k//g'`
			do
	...
	clear
	echo "Best IPs are $first, $second and $third!"
	echo $anycast meets your $bandwidth Mbps speed demand, caps at $max kB/s. Whole test took $((end_seconds-start_seconds)) seconds to complete.